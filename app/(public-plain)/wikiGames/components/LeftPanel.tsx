...
import Checkbox from '../../../../assets/public-components/Checkbox'

import { GameStates } from '@/app/(public-plain)/wikiGames/components/states'

const LeftPanel = ({ gameData }: GameStates) => {
    return (
        <div className="hidden lg:flex 2xl:w-[500px] 3xl:w-[550px] flex-col justify-between items-center gap-6">
            <div className="w-full flex flex-col justify-between items-center border border-[#6E6E6E] rounded-xl p-5">
                <div className="w-full flex justify-between items-center py-2 border-b border-[#6E6E6E] text-[20px] gap-5">
                    <p className="text-[#6E6E6E]">مرحله:</p>
                    <div className="flex items-center gap-1">
                        <PersianNumber value={3} className="text-[#00E489]" />
                        <PersianNumber value="از 10" className="text-[16px] text-[#6E6E6E]" />
                    </div>
                </div>
                <div className="w-full flex justify-between items-center py-2 border-b border-[#6E6E6E] text-[20px] text-[#6E6E6E] gap-5">
                    <p>انرژی:</p>
                    <svg
                        width="100%"
                        height="24"
                ...
                <div className="w-full flex justify-between items-center py-2 text-[20px] text-[#6E6E6E] gap-5">
                    <p>شانس:</p>
                    <Chances lives={gameData.lives} />
                </div>
            </div>
            <div className="w-full flex flex-col gap-5 justify-between items-start border border-[#6E6E6E] rounded-xl p-5">
                <h4 className="text-white">اطلاعات بازی</h4>
                <div className="w-full flex flex-col justify-between items-center">
                    <div className="w-full flex justify-between items-center py-2 border-b border-[#6E6E6E] text-[20px] gap-5">
                        <p className="text-[#6E6E6E]">امتیاز این مرحله:</p>
                        <PersianNumber
                            value={gameData.score}
                            className="text-[#00E489]"
                            comma={true}
                        />
                    </div>
                    <div className="w-full flex justify-between items-center py-2 border-b border-[#6E6E6E] text-[20px] text-[#6E6E6E] gap-5">
                        <p>بالاترین امتیاز شما:</p>
                        <PersianNumber value="20000" comma={true} />
                    </div>
                    <div className="w-full flex justify-between items-center py-2 text-[20px] text-[#6E6E6E] gap-5">
                        <p>مجموع امتیاز های شما:</p>
                        <PersianNumber value="230176" comma={true} />
                    </div>
                </div>
            </div>
            <div className="w-full flex flex-col gap-5 justify-between items-start border border-[#6E6E6E] rounded-xl p-5">
                <h4 className="text-white">تنظیمات بازی</h4>
                <div className="w-full flex flex-col justify-between items-center">
                    <div className="w-full flex justify-between items-center py-2 border-b border-[#6E6E6E] text-[20px] gap-5">
                        <p className="text-[#6E6E6E]">موسیقی پس‌زمینه:</p>
                        <Checkbox
                            defaultChecked
                            checkboxClass="mute flex items-center rounded-full text-[#00E489]"
                        ></Checkbox>
                    </div>
                    <div className="w-full flex justify-between items-center py-2 text-[20px] text-[#6E6E6E] gap-5">
                        <p>صدای افکت:</p>
